...
// same order: flip → tile → zoom/pan → brcosa + edge-blend → blur → motion-blur.
// (Mask and blend-mode compositing stay in the LayerStack blend pass; per-screen mesh
// warp stays in ScreenWarp.)
//
...
export class FxChain {
  ...
  process(srcTexture, fx, { isColor = false, color = [0, 0, 0] } = {}) {
  ...
  dispose() {
    const gl = this.gl;
    for (const fbo of [this.pointFbo, ...(this.blurFbo ?? []), ...(this.feedbackFbo ?? [])]) {
      if (!fbo) continue;
