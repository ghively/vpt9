...
- Render client (one per physical screen): `http://localhost:8081/index.html?screen=screen-1&ws=ws://localhost:8080`
  — composites at the output's own pixel resolution (dpr-aware, capped at 4096); append
  `&res=1920x1080` to pin a fixed internal resolution instead (a relief valve for weak
  projector GPUs, or to force a show's master resolution). The corner status line shows
  while connecting/disconnected and hides itself a moment after connecting — nothing is
  overlaid on a live projection.
