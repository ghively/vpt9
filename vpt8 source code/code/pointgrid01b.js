...
function create_points(gridsize){
	
    var x=0.;
    var y=0.;
	var active=0;
	var cur=0;
	var relpos=2/(gridsize-1);

    for(i = 0; i < gridsize ; i++){
		for(j = 0; j < gridsize ; j++){
			cur=j+(gridsize*i);
			outlet(1,cur);
        points[cur] = new point(x,y,active);
		points[cur].x=-1+(i*relpos);
		points[cur].y=-1+(j*relpos);
       }
	 }
       
    }

function draw_lines(gridsize) {
	var cur=0;
	var len=grid*grid;
	...
	outlet(0,"set_source_rgba",1.0, 1.0, 1.0, .7);
		...
		for(j = 0; j < gridsize ; j++){
			cur=j+(grid*i);
			outlet(1,cur);
			outlet(0,"move_to",points[cur].x, points[cur].y);
			if (j < gridsize-1) {
			outlet(0,"line_to",points[cur+1].x, points[cur+1].y);
			}
			outlet(0,"move_to",points[cur].x, points[cur].y);
			if (i < gridsize-1) {
			outlet(0,"line_to",points[cur+grid].x, points[cur+grid].y);
			}			
       }
...
function print_points() {
		...
		for(i = 0; i < len ; i++){
		outlet(1,i);
		outlet(1,"x,y",i,points[i].x,points[i].y);
...
function gridlist() {
		...
		var len=grid*grid;
		outlet(2,"/gridsize",len);
		for(i = 0; i < len ; i++){
		outlet(2,"/position",i,points[i].x, points[i].y);
       }
		outlet(2,"/trig bang");
	}
...
function savedgrid() {
	...
	var len=len/3; //the size of the array of coordinates
	points.length=0; //empty original array of points
	
	for(i = 0; i < len ; i++){
		points[i] = new point(0.,0.,0);		
		points[i].x=temparray[(i*3)+1];
		points[i].y=temparray[(i*3)+2];
		points[i].active=0;
	}
	draw();
	print_points();
